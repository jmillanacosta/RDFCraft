...
type WorkspaceItemStoreActions = {
    ...
    addMapping: (name: string, description: string, file: File) => void;
    ...
    devtools(
        (set, get) => ({
            ...
            fetchById: async (id: string) => {
                ...
                try {
                    const workspace = await WorkspaceService.fetchWorkspaceById(
                        id,
                    );
            ...
            addPrefix: async (prefix: string, uri: string) => {
                ...
                try {
                    ...
                    const response =
                        await WorkspaceService.addPrefixToWorkspace(
                            get().workspace!._id,
                            model,
                        );
                    if (!response.success) {
            ...
            addBulkPrefix: async (models: PrefixModel[]) => {
                ...
                try {
                    const futures = models.map((model: any) =>
                        WorkspaceService.addPrefixToWorkspace(
                            get().workspace!._id,
                            model,
                        ),
                    );
            ...
            removePrefix: async (prefixId: string) => {
                ...
                try {
                    const response =
                        await WorkspaceService.removePrefixFromWorkspace(
                            get().workspace!._id,
                            prefixId,
                        );
                    if (!response.success) {
            ...
            addOntology: async (
                ...
                try {
                    const response =
                        await WorkspaceService.addOntologyToWorkspace(
                            get().workspace!._id,
                            name,
                            description,
                            prefix_id,
            ...
            deleteOntology: async (id: string) => {
                ...
                try {
                    const response =
                        await WorkspaceService.removeOntologyFromWorkspace(
                            get().workspace!._id,
                            id,
                        );
                    if (!response.success) {
            ...
            addMapping: async (
                ...
                try {
                    const response =
                        await WorkspaceService.addMappingToWorkspace(
                            get().workspace!._id,
                            name,
                            description,
                            file,
            ...
            deleteMapping: async (id: string) => {
                ...
                try {
                    const response =
                        await WorkspaceService.removeMappingFromWorkspace(
                            get().workspace!._id,
                            id,
                        );
                    if (!response.success) {
