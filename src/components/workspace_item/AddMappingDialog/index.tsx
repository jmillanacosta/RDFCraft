...
type _WorkspaceAddMappingProps = {
  ...
  loading: boolean;
  onClose: () => void;
  onConfirm: (name: string, description: string, file: File) => void;
};
          ...
          onSubmit={async e => {
            ...
            try {
              if (state.sourceType === 'json') {
                ...
                json = JSONPath({ path: state.jsonPath!, json });
                if (json.length === 0) {
                  setState({
                    ...state,
            ...
            onConfirm(state.name, state.description, file!);
          }}
        >
          <DialogContent>
            <TextField
            ...
            <TextField
              ...
              disabled={loading}
              sx={{ display: state.sourceType === 'json' ? 'block' : 'none' }}
            />
