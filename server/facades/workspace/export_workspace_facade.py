...
class ExportWorkspaceFacade(BaseFacade):
    def __init__(
        ...
        self.ontology_service: OntologyServiceProtocol = (
            ontology_service
        )
    ...
    def execute(
        ...
        self.logger.info(
            f"Exporting workspace {workspace_id}"
        )

        self.logger.info(
            f"Getting workspace metadata {workspace_id}"
        )

        workspace_metadata: WorkspaceMetadata = self.workspace_metadata_service.get_workspace_metadata(
            workspace_id
        )

        self.logger.info(
            f"Getting workspace {workspace_id}"
        )
        ...
        self.logger.info(
            f"Getting mappings for workspace {workspace_id}"
        )
        ...
        self.logger.info(
            f"Getting sources for workspace {workspace_id}"
        )

        sources = [
            self.source_service.get_source(
                mapping.source_id
            )
        ...
        self.logger.info(
            f"Getting ontologies for workspace {workspace_id}"
        )
        ...
        self.logger.info(
            f"Getting files for workspace {workspace_id}"
        )
        ...
        self.logger.info(
            f"Creating export metadata for workspace {workspace_id}"
        )
        ...
        self.logger.info(
            f"Tar file path for mapping {workspace_id}: {tar_file_path}"
        )
