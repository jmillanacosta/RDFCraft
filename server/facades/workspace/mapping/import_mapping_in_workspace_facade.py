...
class ImportMappingInWorkspaceFacade(BaseFacade):
    ...
    def execute(
        ...
        self.logger.info(
            f"Workspace {workspace_id} retrieved"
        )

        self.logger.info(f"Extracting tar")

        tar_f: tarfile.TarFile = tarfile.open(
            fileobj=BytesIO(tar)
        )

        export_metadata_raw = tar_f.extractfile(
            "metadata.json"
        )
        ...
        export_metadata = ExportMetadata.from_dict(
            json.loads(
                export_metadata_raw.read().decode("utf-8")
            )
        )

        if (
            export_metadata.type
            != ExportMetadataType.MAPPING
        ):
            raise ServerException(
        ...
        self.logger.info(
            f"Importing mapping {export_metadata.mappings[0].name}"
        )

        self.logger.info(
            "Dumping mapping jsons to files storage"
        )
        ...
        self.file_service.upload_file(
            name=source.uuid,
            uuid=source.uuid,
            content=json.dumps(source.to_dict()).encode(
                "utf-8"
            ),
        )
        self.file_service.upload_file(
            name=mapping.uuid,
            uuid=mapping.uuid,
            content=json.dumps(mapping.to_dict()).encode(
                "utf-8"
            ),
        )
        for file in export_metadata.files:
            file_raw = tar_f.extractfile(
                f"files/{file.uuid}"
            )
            if file_raw is None:
                raise ServerException(
        ...
        self.logger.info(
            f"Mapping {mapping.name} imported, registering in workspace"
        )
        ...
        return self._success_response(
            data=None, message="Mapping imported"
        )
