...
class RMLMapperService(RMLMapperServiceProtocol):
    def __init__(
        ...
        if custom_java_path:
            self.logger.info(
                f"Using custom Java path: {custom_java_path}"
            )
            try:
                subprocess.run(
                ...
                self.logger.error(
                    "Java not found, trying default Java path"
                )
        ...
        self.logger.info(
            f"Java found at {self.java_path}, checking for mapper"
        )
    ...
    def execute_rml_mapping(self, mapping: str) -> str:
        ...
        rml_file: Path = (
            self.TEMP_DIR / f"rml_{process_uuid}.ttl"
        )

        rml_file.touch()
        rml_file.write_text(mapping)

        rdf_output_file: Path = (
            self.TEMP_DIR / f"rdf_{process_uuid}.ttl"
        )
        ...
        self.logger.info(
            "RML mapping executed successfully"
        )
