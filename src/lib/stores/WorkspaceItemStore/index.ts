...
const functions: ZustandActions<
    WorkspaceItemStoreActions,
    WorkspaceItemStoreState
> = (set, get) => ({
    ...
    addMapping: async (name: string, description: string, file: File) => {
        set({ loading: true, error: null });
        if (!get().workspace) {
            set({
                error: 'Workspace not found, please go back and try again',
        ...
        try {
            const response = await WorkspaceService.addMappingToWorkspace(
                get().workspace!._id,
                name,
                description,
                file,
    ...
    deleteMapping: async (id: string) => {
        ...
        try {
            const response = await WorkspaceService.removeMappingFromWorkspace(
                get().workspace!._id,
                id,
            );
            if (!response.success) {
